...
imageDir = r'C:\Users\maxwe\Desktop\My Documents\MathExprSolverMx\MathExprSolverMx\AidaCalculusHandWrittenMathDataset\archive\train\images'
annoDir = r'C:\Users\maxwe\Desktop\My Documents\MathExprSolverMx\MathExprSolverMx\AidaCalculusHandWrittenMathDataset\archive\train\annotations'
...
startImage = 140;
for n in range(startImage,len(imgFiles)):
    imgName = imgFiles[n]
    imgbaseName = imgName[:-4]
    annoFile = imgbaseName + '.json'
    
    if imgbaseName[-8:] != 'bkgrdImg':
    
        imgFile = os.path.join(imageDir, imgName)
        annoFile = os.path.join(annoDir, annoFile)
        
        with open(annoFile) as f:
            annoData = json.load(f)
            
        annoData.keys()
        annoData['image_data'].keys()
        
    
        # Open the test image.
        example_img = Image.open(imgFile)
        
        # Gather appropriate items from the data json for the image.
        xmins = annoData["image_data"]["xmins_raw"]
        ymins = annoData["image_data"]["ymins_raw"]
        xmaxs = annoData["image_data"]["xmaxs_raw"]
        ymaxs = annoData["image_data"]["ymaxs_raw"]
        labels = annoData['image_data']['visible_latex_chars']
    
        # Unpack and convert serialized pngs.
        masks = unpack_list_of_masks(annoData["image_data"]["png_masks"])
